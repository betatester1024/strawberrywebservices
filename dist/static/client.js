...
function nameSearch() {
    ...
    send(JSON.stringify({ action: "strawbCmd", data: { cmd: "NameSearch", val: name } }), (res) => {
        ...
        if (value.entries == null)
            alertDialog("Error in search, check console.");
        else {
            console.log(value);
...
function dispShipSummary(outputCtn, ships) {
...
function setTab(id) {
    if (!id || byId(id) == null || byId("t-" + id) == null)
        id = "networth";
...
function calcWorth() {
    ...
    send(JSON.stringify({ action: "strawbCmd", data: { cmd: "ValueTotal", shiplist: area.value } }), (res) => {
        ...
        if (initMatch == null)
            alertDialog("Invalid shiplist data.", () => { });
        else {
            byId("resCtn").style.display = "block";
            let breakdownEle = byId("breakdown");
...
let page = 1;
function getLeaderboard(delta) {
    if (delta == 0)
        page = Number(byId("inp-page").value);
    else
        page += delta;
    if (page < 1)
        page = 1;
